...
export default function Dashboard() {
  ...
  async function getBlogItems(): Promise<void> {
    try {
      const response = await axios.get(`http://localhost:3000/api/userBlog`);
  ...
  const routetoBlog = (blogId: string) => {
    router.push(`/dashboard/${blogId}/readItem`);
  };
            ...
            blogItems.map((post) => (
              <article key={post._id} className="flex flex-col bg-blue-400 p-5 rounded-lg lg:flex-row items-start gap-x-6">
                ...
                <div className="flex flex-col justify-between">
                  ...
                  <div className="mt-6">
                    <Link href={`dashboard/${post._id}/readItem`} className="text-blue-500 hover:text-blue-700">
                      Read More
                    </Link>
